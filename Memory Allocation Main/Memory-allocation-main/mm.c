...
void* mm_malloc (size_t size) {
  ...
  if (ptrFreeBlock == NULL) {
    // No free block was found, so we need to request more space.
    requestMoreSpace(reqSize);
    ptrFreeBlock = searchFreeList(reqSize);
  }
  removeFreeBlock(ptrFreeBlock);

  // We now have a free block that is big enough.
  // We need to split it if it is too big.
  ...
  if (remainderSize >= MIN_BLOCK_SIZE) {
    // Split the block.
    // The first part of the block is the requested size.
    // The second part of the block is the remainder.
    ptrFreeBlock->sizeAndTags = reqSize | precedingBlockUseTag | TAG_USED;
    BlockInfo *ptrRemainderBlock = (BlockInfo*)UNSCALED_POINTER_ADD(ptrFreeBlock, reqSize);
    ptrRemainderBlock->sizeAndTags = remainderSize | TAG_PRECEDING_USED;
    // boundary tag
    *((size_t*)UNSCALED_POINTER_ADD(ptrRemainderBlock, remainderSize - WORD_SIZE)) = remainderSize | TAG_PRECEDING_USED;
    // Add the remainder to the free list.
    insertFreeBlock(ptrRemainderBlock);
    coalesceFreeBlock(ptrRemainderBlock);
  } 
  else {
    // update block header to be used
    ptrFreeBlock->sizeAndTags |= TAG_USED;
    BlockInfo *ptrNextBlock = (BlockInfo*)UNSCALED_POINTER_ADD(ptrFreeBlock, blockSize);
    ptrNextBlock->sizeAndTags |= TAG_PRECEDING_USED;
  }

  // Return a pointer to the user data portion of the block.
...
void mm_free (void *ptr) {
  ...
  size_t * footer = (size_t*)UNSCALED_POINTER_ADD(blockInfo, payloadSize - WORD_SIZE);
  *footer = blockInfo->sizeAndTags;
